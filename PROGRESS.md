...
- **풀스택 질의 문서**: FQ1~9로 작성했다가 팀장 전달 메시지(11건)·
  `풀스택_문서_반영사항_2026-07-27.md`와 대조해 **중복 5건 제거**, 남은 4건을 팀장
  번호를 이어받아 **12~15번 보완**으로 재구성.
  고유 논점: 제한시간·동시요청량(실측 첨부) · callback_url · 콜백 필드명/값 형식 ·
  detail 조합(요청자 노출 여부 — 개인정보). 전달은 사용자 몫.
  → 이후 **`docs/풀스택_회신요청.md` 하나로 통합**됐다(2026-07-31 문서 정리). 당시의
  12·14·15번은 통합본의 **8·9·10번**, 13번(callback_url)은 **부록 C3 행**에 들어갔다.
