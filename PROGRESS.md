...
- **카테고리 불일치 가드레일 (d41fba1)** ⚠️ 팀 결정 변경 — 팀장 공유 필요:
  ...
  확인(풀스택 Q9) 후 '목록 밖 값=비정상 입력→보류'로 강화 예정.**
- **심사 소요시간 실측 (7/28, 표본 9건)**: 그래프 중앙값 3.3s·최대 4.4s —
  백엔드 콜백 예산(bravo "예: 30초") 대비 여유. 단일 워커 순차 처리라 동시
  7~8건이 한계 → 워커 증설은 예상 동시량(풀스택 Q8) 확인 후.
- **환경 정비 (7/28)**: 도커가 7/22 실모드 실험 잔재로 떠 있던 것 목 모드
  재빌드(핸드오프 경고 항목), demo-% 팀의 실임베딩 잔재 청크 26개 삭제(목 검색과
  좌표계 불일치로 rule_ambiguous 오탐 유발). **OpenAI 키 quota 소진 사건**:
  7/28 17:19 워커에서 429 insufficient_quota 연속(심사관 error→escalate로
  fail-safe 정상 동작), 이후 회복 확인 — 실모드 검증 전 키 잔액 확인 습관 필요.
- **풀스택 질의 문서 Q1~Q9 작성**: 필드명 대조(verdict↔finalVerdict 등)·자동승인
  주체 문서 상충·callback_url·30초 제한·카테고리 명칭 통일/기타 추가. 파일은
  기획 폴더(바탕화면) 보관 중 — 전달은 사용자 몫.
