...
# ── 1. 문서와 코드가 어긋나지 않았는가 ────────────────────────────────────
r = subprocess.run([sys.executable, str(ROOT / "scripts" / "dump_openapi.py"), "--check"],
                   capture_output=True, text=True, encoding="utf-8", errors="replace")
check("OpenAPI 스펙이 코드와 일치", r.returncode == 0,
...
bad = [i for i, s in enumerate(spec.few_shot, 1)
       if s["input"].rstrip("\n").split("\n")[0] != header]
check(f"분류기 few_shot이 카탈로그와 일치 ({spec.version})", not bad,
...
copied = [ln.split()[1] for ln in dockerfile.splitlines()
...
check("이미지에 app·prompts·templates 포함", not missing,
...
r = subprocess.run([sys.executable, "-m", "pytest", "-q"], cwd=ROOT, env=env,
                   capture_output=True, text=True, encoding="utf-8", errors="replace")
