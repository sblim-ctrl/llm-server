...
| **위치** | **`풀스택_문서_반영사항_2026-07-27.md` §3-1(내부 Agent API 명세 요청)의 구체안** — 그 문서가 "8종이 명세에 없으니 열어달라"고 요청했다면, 이 문서는 **각 API의 경로·파라미터·응답 예시를 제안**한다. 함께 보아야 한다. |
| 함께 볼 것 | `풀스택_연동_계약.md`(전달 메인) · `풀스택_문서_반영사항_2026-07-27.md`(회신 필요 논점) · `풀스택_전달_메시지.md`(회신 요청 11건) |
...
- **전송 payload** — 아래는 `CallbackPayload`를 실제 직렬화해 확인한 출력이다
  ...
  ```
- ⚠️ **파서 작성 시 주의 2건**(현행 출력 기준 — `풀스택_전달_메시지.md` 11번으로 회신 요청 중):
