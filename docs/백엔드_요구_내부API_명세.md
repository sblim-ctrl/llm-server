...
| 항목 | 내용 |
|---|---|
| 작성일 | 2026-07-29 (팀장 리뷰 반영 갱신) · 2026-08-05 (⑪ 지출 이력 조회 누락 복원) · **2026-08-06 (풀스택 회신 반영 — ① `category` 정책 · ② `escalation_threshold` 반영 완료 · ⑤ 회칙 파일 응답 판별 규칙 신설 · ⑪ 신설 확정)** |
| 작성 | LLM팀 (개발자 A) |
...
- **`escalation_threshold` 컬럼은 삭제한다** (2026-08-05 합의). 화면 금액 칸이 2개에서
  ...
  - ✅ **우리 쪽 반영 완료 (2026-08-05, PR #12 머지).** 키가 없으면 `auto_approve_limit`과
    같은 값으로 읽습니다. **다만 코드 배포는 8/7 예정**이므로, 컬럼 삭제는 그 뒤로 잡아
    주세요. 배포가 끝나면 컬럼이 있든 없든 양쪽 다 정상 동작합니다
  - 🟡 **컬럼이 남아 있는 동안의 조건**: `auto_approve_limit`과 `escalation_threshold`에
    **같은 값**(마법사 2단계 기준 금액)이 들어 있어야 합니다. 한쪽에 옛 값(예: 200,000)이
    남으면 실효 한도가 `min(두 값)`이라 위와 같은 조용한 축소가 납니다
...
- 파싱 실패 사유는 `GET /v1/jobs/{job_id}`의 `result`에 관리자에게 그대로 보여드려도 되는
  문장으로 담깁니다. 예: *"스캔한 이미지 PDF는 글자를 인식할 수 없으니, 텍스트가 들어 있는
  PDF나 Word 파일로 다시 올려 주세요."*
- 어떤 파일인지는 LLM-005 요청의 `rule_file_ref`로 지정됩니다
...
- 🟡 **`category` 값 주의**: ENUM 마이그레이션 전 데이터에는 구 7종(`디자인`·`행사` 포함)이
  섞여 있습니다. 9종 밖 값이 그대로 오면 **리포트·대시보드 집계가 조용히 어긋납니다.**
  이관 대상 회신을 기다리는 중이며, 저희 제안은 `행사` → `행사_활동`, `디자인` → `기타`입니다
