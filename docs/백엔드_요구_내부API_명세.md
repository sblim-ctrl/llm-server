...
- `escalation_threshold`는 금액으로 확정됐다(2026-07-27 DB 스키마 `team_settings` 근거).
  ...
  코드 반영을 마쳤고 확신도 임계값 θ(0~1 실수)와는 분리했다. 명칭은 `escalationThreshold`로
  통일한다. 목 구현이 아직 `0.8`을 반환하는 것은 우리 쪽 잔여 정리 사항이며 회신이 필요한
  항목이 아니다
- 회신 필요: 이 값의 기본값을 확정해 주세요. AI 마법사 2단계 화면이 200,000원 기준으로
