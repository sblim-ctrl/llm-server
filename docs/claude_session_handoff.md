...
- 🔴 **배포가 2026-08-07(금)이다.** GCP. (8/6으로 적혀 있던 것을 팀장님 작업리스트
  `LLM팀_작업리스트_MVP_2026-08-05.md` 기준으로 2026-08-05에 정정)
...
카테고리도 9종으로 바꿔야 한다. 지금 골든셋 카테고리(`다과`·`대관`·`도서`·`용품` 등)가
카탈로그 밖 어휘라 **채점 기준이 서지 않는다.**

> ⚠️ **2026-08-06 정정 (T7).** 종전 서술은 "카탈로그 밖 값이면 `classify_category.py:56`에서
> 즉시 반환돼 분류기가 한 번도 호출되지 않는다"였다. **T7이 그 조기 반환을 없앴다** —
> 이제 어떤 값이 와도 항상 분류하고, 들어온 값은 AI 결과로 덮인다. 따라서 골든셋의
> `category`는 "입력"이 아니라 **채점용 기대값(`expected_category`)** 으로 다뤄야 한다 —
> T9 fixture 설계의 전제가 바뀐 지점이다. 읽기 경계 정규화(`0155ad9`·`3b70de5`)도 함께
> 볼 것: 실모드 응답의 구 값은 이제 9종으로 접히고, 빈 값·목 응답은 접지 않는다.
