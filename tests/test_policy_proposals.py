...
from app.api.drafts import (
    ...
    read_policy_proposal,
)
from app.schemas.writers import PolicyDraft, PolicyProposalRequest
...
REQ = PolicyProposalRequest(
    ...
    force_escalation_amount=50_000,  # rule_source는 "ai" 고정이라 생략 가능
)
...
async def test_reuses_existing_open_proposal_instead_of_regenerating():
    ...
    with (
        patch("app.api.drafts.list_proposals", AsyncMock(return_value=[_row("p-1", saved)])),
...
async def test_decided_proposal_does_not_block_new_generation():
    ...
    with (
        patch("app.api.drafts.list_proposals", AsyncMock(return_value=[done])),
        patch("app.api.drafts._generate", AsyncMock(return_value=_draft("새 조항"))),
        patch("app.api.drafts.save_proposal", AsyncMock(return_value="p-2")),
    ):
        out = await create_policy_proposal(REQ)
...
async def test_read_returns_open_proposal():
    with patch("app.api.drafts.list_proposals", AsyncMock(return_value=[_row("p-1", _draft())])):
...
async def test_read_skips_decided_and_takes_latest_open():
    ...
    rows = [
        _row("p-3", _draft("최신"), status="dismissed"),
...
def test_request_carries_the_same_fields_as_wizard_draft():
    """초안을 만드는 재료는 마법사 통합 요청과 같다 — 달라질 이유가 없다.

    to_draft_request는 model_dump 전체를 넘기므로(regenerate만 제외), 통합 요청에
    필드가 늘어도 여기서 조용히 빠지지 않는다.
    """
    d = REQ.to_draft_request()
    assert d.team_id == 9001
    ...
    assert d.force_escalation_amount == 50_000
    assert d.rule_source == "ai"
...
def test_request_pins_rule_source_to_ai():
    """관리 화면의 목적은 AI 초안 생성 — 다른 rule_source를 열면 빈 초안이 저장된다."""
    with pytest.raises(ValueError):
        PolicyProposalRequest(
            team_id=9001,
            team_type="스터디",
