...
DEFAULT_VERSIONS = {
    ...
    "classifier": "v7",      # 전역 9종 대응(v3) → 물건 형태의 교육 지출(v4) →
                             #  활동 용품 vs 활동 참가 경계(v5) → 교육 참가비 경계(v7).
                             #  전부 실모드 A/B 승격 (각 2회 재현. 하니스 scripts/ab_classifier.py):
                             #    v3 11/14(78.6%) → v4 13/14(92.9%)  회귀 0
                             #    v4 14/17(82.4%) → v5 17/17(100%)   회귀 0
                             #    v6 19/24(79.2%) → v7 23/24(95.8%)  회귀 0 (2026-08-10)
                             #  v5·v7 모두 개선분에 few_shot에 없는 표현이 섞여 있다 —
                             #  암기가 아니라 규칙 학습이라는 근거다(v5: 풋살 유니폼·농구공,
                             #  v7: 주말 코딩 부트캠프·신입생 오리엔테이션 세미나).
                             #  v7이 고친 것: '참가비'가 강연·강습 같은 교육 콘텐츠에 붙을 때
                             #  v6은 전부 행사_활동으로 보냈다(실모드 골든셋 club-approve-004·
                             #  social-approve-003 오분류의 원인). 반대 방향(체육대회·페스티벌
                             #  참가비 → 행사_활동)은 두 회차 모두 그대로 유지됐다.
    ...
    "query_rewriter": "v2",  # 정합 수정(2026-08-07): 예시 라벨 '회식비'·'도서' → 식비·교육.
                             ...
                             #  어휘 자유 텍스트라 그대로 뒀다.
    "rule_amendment": "v2",  # 실측 A/B 승격이 아니라 **정합 수정**이다(2026-08-07). v1은
                             #  2026-08-04 브랜치 통합 때 이미 sblim에만 있던 버전으로 파악됐지만
