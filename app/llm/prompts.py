...
DEFAULT_VERSIONS = {
    ...
    "classifier": "v3",      # 전역 9종 카테고리 대응 (풀스택 협의 2026-08-04).
                             #  실측 승격이 아니라 **정합성 수정**이다 — v1 few_shot이
                             #  이제 존재하지 않는 라벨("도서·강의·다과")을 가르치고
                             #  user 메시지 형식도 달라져(모임 유형 줄 제거) v1로는
                             #  분류가 성립하지 않는다. 골든셋이 깨져 있어(§4-1) A/B로
                             #  잴 수 없는 상태라, 되살아나면 v1 대비 실측할 것.
