...
기본 조항 중 금액 한도가 든 것은 근거로 쓰지 않는다.
...
남는 것은 성격 조항(영수증 첨부·개인 용도·목적 무관 등)인데, 이건 유형과 무관하게
대체로 통용되는 상식에 가까워 기본값으로 적용해도 무리가 없다.
