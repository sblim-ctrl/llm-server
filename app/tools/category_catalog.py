...
def keyword_category_or_none(text: str) -> str | None:
    ...
    '기타' 재질의(classify_category)의 핵심 경로다 (T7, 2026-08-06): 분류기가
    '기타'라고 답했을 때 이 함수가 아는 것이 있으면 그 값이 **최종 카테고리로
    확정**된다. fallback을 돌려주면 "모른다"와 "기타가 맞다"를 구분할 수 없어
    재질의가 성립하지 않으므로, 적중했을 때만 값을 준다. 순수 함수 (단위 테스트
    대상). (원래 용도였던 가드레일 category_mismatch 비교는 T7로 제거됐다.)

    매칭은 부분 문자열이고 YAML에 적힌 순서대로 먼저 걸리는 카테고리가 이긴다 —
    그래서 호출부는 **사용자가 쓴 제목·설명만** 넣어야 한다. 영수증 상호명은 업종과
    무관한 낱말을 흔히 품어('플라워카페'의 카페→식비) 부분 문자열에 안전하지 않다(B-1).
    """
    lowered = text.lower()
