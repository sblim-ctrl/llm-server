...
def merge_opinions(
    left: dict[str, Opinion] | None, right: dict[str, Opinion] | None
) -> dict[str, Opinion]:
    return {**(left or {}), **(right or {})}


def merge_llm_meta(
    left: dict[str, LLMCallMeta] | None, right: dict[str, LLMCallMeta] | None
) -> dict[str, LLMCallMeta]:
    return {**(left or {}), **(right or {})}
...
class ReviewState(TypedDict, total=False):
    ...
    # 분류 (classify_category가 씀) — 항상 "ai"다. 사용자 카테고리 입력이
    # 화면에서 사라져(8/4 회의) AI 분류가 유일한 출처가 됐다 (T7, 2026-08-06).
    # 사용자 선택과 대조하던 category_mismatch·ai_suggested_category 필드는
    # 가드레일 규칙·테스트와 함께 제거했다 (2026-08-06 팀장 승인 — 세우는 코드 0곳).
    category_source: str
    # 진행 산출물
