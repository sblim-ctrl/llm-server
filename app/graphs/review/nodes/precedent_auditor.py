...
def _precedent_lines(cases: list[dict]) -> str:
    """판례 인용 라인 (순수 함수) — (결정/결정주체) 표기로 ADMIN/AGENT·override를 노출.

    프롬프트(v2+)의 "ADMIN 판례만 근거" 규칙은 이 표기에 의존한다 — 표기 없이는
    모델이 결정주체를 구분할 입력 자체가 없다. precedent_auditor/v2+ few_shot의
    판례 라인이 이 형식을 미러링한다 — 형식 변경 시 함께 갱신.
    """
    if not cases:
        return "(없음)"
    return "\n".join(
        f"- ({c['decision']}/{c['decided_by']}{', override' if c['is_override'] else ''}) "
        f"{c['expense_summary']} — 사유: {c['reason'] or '사유 없음'}"
        for c in cases
    )
...
async def precedent_auditor(state: ReviewState) -> dict:
    ...
    try:
        ...
        cases = await search_precedents(
            state["team_id"], await masked_claim_summary(state["team_id"], claim)
        )
