...
# A-9 실키 실측(2026-07-20, text-embedding-3-small 코사인 거리)으로 처음 보정:
# 관련 조항 0.42-0.51 / 무관 조항 0.71+ / 완전 무관 0.81+ — 구값 0.5는 관련 조항
# (교재→도서 조항 0.509)을 경계에서 놓쳤다. 임베딩 모델 교체 시 재실측 필수.
#
# **0.65 → 0.75 (2026-08-13, 조항 라벨 82건 실측)**. 종전 값은 표본 5건 기준이었고,
# 라벨을 90건으로 늘려 재보니 **정답 조항을 1위로 찾아놓고 문턱에서 버리는** 케이스가
# 많았다: study-reject-001 제10조 1위 d=0.676 · hobby-reject-001 제6조 1위 d=0.704.
# 0.65를 근소하게 넘겨 탈락한 것이라 "못 찾은" 게 아니라 "찾고도 버린" 것이다.
#
# 후보 수(top_k)를 3→10으로 늘려도 적중은 +1건뿐이었다 — 병목은 후보 수가 아니라
# 이 문턱이었다. 24조합 스윕에서 0.75가 적중과 노이즈의 균형점이다:
#   0.65: 적중 81.7% · 노이즈 0.6개   0.75: 적중 93.9% · 노이즈 1.5개
#   0.80: 적중 95.1% · 노이즈 1.9개   (0.80 이상은 적중이 평평해지고 노이즈만 증가)
#
# 종단 검증(골든 97건 실모드): 판정 정확도 97.9% → **100%**, 오승인 0 유지,
# 에스컬레이션 정밀도 96% → 100%. 근거를 못 찾아 보류하던 2건이 조항을 받자 스스로
# 판정했다(hobby-gate-priority-001 → reject, company-budget-warn-001 → approve).
# 노이즈가 늘어 오판이 생길 위험을 걱정했는데 실측에서는 반대로 나왔다.
#
# 대가: 회칙이 다루지 않는 지출(대조군 8건)에서 무관 조항이 근거로 올라오는 비율이
# 7/8 → 8/8이 된다. 심사관이 "해당 없음"으로 판단하는 것은 정상 동작이고(Self-RAG)
# 하니스도 경고로만 세지만, 프롬프트가 무관 조항에 끌려가는지는 계속 봐야 한다.
