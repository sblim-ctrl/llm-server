...
# 잔액이 남았는데 "다 썼다"고 하는 과장 표현.
#
# **완료형만 매칭한다** — 처음엔 `(사용|썼|소진)` 뒤에 부정 선읽기로 가정형을 제외했는데,
# 그 방식은 `사용` 바로 뒤만 봐서 어미 6가지가 뚫렸다("사용하게 되면"·"사용한다면"·
# "사용하려면"·"사용해도"·"사용하기 전"·"사용하게 될" — PR #11 리뷰 A-7 지적).
# 띄어쓰기가 끼면 `\S*`로도 못 넘는다. 그래서 "조건형을 빼는" 대신 "완료형만 잡는"
# 쪽으로 뒤집었다. 가정·미래형은 애초에 매칭되지 않는다.
#
# 완료형 목록은 격식체·수동태 변형까지 포함한다("사용하였습니다"·"소진했습니다"·
# "사용되었습니다"가 빠져 있던 것을 실측으로 확인해 추가). 단 완료형 자체에 붙는
# 가정·추측 어미(했**다면**·했**더라면**·했**을** 수도)는 주장이 아니므로 뒤를 한 글자만
# 선읽기로 제외한다 — 어간과 어미는 한 낱말이라 띄어쓰기가 낄 수 없어, 첫 구현의
# "사용 바로 뒤만 본다" 문제가 여기서는 생기지 않는다.
#
# 측정: 과장 문장 13개 전건 탐지 · 정상 문장 18개 오탐 0 — 측정 문장 전체가
# tests/test_dashboard_summary.py의 과장 그물 테스트에 그대로 있다.
_OVERSTATE_RE = re.compile(
    r"(모두|전부|전체|전액|다)\s*"
    r"(사용했|사용하였|사용됐|사용되었|사용함|사용됨|소진했|소진하였|소진됐|소진되었|소진함|소진됨|썼)"
    r"(?!다면|더라면|을)"
    r"|예산이\s*(없|바닥)"
    r"|남은\s*(예산|금액)이\s*없"
)
...
def verify_summary_pure(doc: DashboardSummaryDoc) -> bool:
    ...
    **숫자는 예외**다. 대시보드는 관리자가 예산 판단을 하는 화면이라, 없는 금액이
    ...
    # `> 0`이다(`!= 0` 아님, PR #11 리뷰 A-7 질문에 대한 답). 잔액이 **음수**면 예산을
    # 넘긴 것이라 "예산을 다 썼다"는 과장이 아니라 사실이고, 오히려 관리자가 가장
    # 알아야 할 말이다. `!= 0`이면 그 사실 보고까지 막혀 예산 초과 달의 요약이
    # 폴백으로 밀려났다.
