...
- **저장소 밖 파일을 근거로 인용할 때는 전체 경로를 밝힌다.** 개인 메모(`claude.llm memo/…`)·gitignore 대상(`docs/_private/…`)은 상대가 볼 수도 grep할 수도 없다. 경로 없이 절·행 번호만 쓰면 상대는 저장소에서 찾다가 "없는 것을 지어냈다"고 결론 낸다 — PR #17에서 실제로 발생했다. 문서마다 절 번호가 겹치므로 "§7"처럼 번호만 쓰는 것도 같은 문제를 만든다.
